...
type BonusRow = {
  date?: string;              // "2025-10-25" (opsiyonel)
  employee_code?: string;     // "RD-101"
  employee_name?: string;     // "Demir"
  count?: number;             // işlem sayısı
  avg_first_sec?: number;     // ort. ilk yanıt süresi (sn)
  avg_close_sec?: number;     // ort. sonuçlandırma (sn)
  approve_rate?: number;      // 0..1 veya 0..100
  reject_rate?: number;       // 0..1 veya 0..100
  total_amount?: number;      // toplam bonus miktarı
  avg_amount?: number;        // ortalama bonus miktarı
  dept?: string;              // "bonus" vb.
  [k: string]: any;
};
...
export default function ReportBonusClose() {
  ...
  useEffect(() => {
    ...
    setErr(null);
    api
      .get<ApiListResponse<BonusRow>>(PATH, {
        from,
        to,
        order,      // ör: "-count" | "avg_close_sec"
        limit,
        offset,
        tz: "Europe/Istanbul",
      })
  ...
  const columns: Column<BonusRow>[] = [
    ...
    { key: "count", header: "İşlem" },
    {
      key: "avg_first_sec",
      header: "Ø İlk Yanıt",
      render: (r) => (r.avg_first_sec != null ? formatSecondsToMmSs(r.avg_first_sec) : ""),
    },
    {
      key: "avg_close_sec",
      header: "Ø Sonuçlandırma",
      render: (r) => (r.avg_close_sec != null ? formatSecondsToMmSs(r.avg_close_sec) : ""),
    },
    {
      key: "approve_rate",
      header: "Onay",
      render: (r) => (
        <b style={{ color: "#0a7" }}>{formatPercent(r.approve_rate)}</b>
      ),
    },
    {
      key: "reject_rate",
      header: "Ret",
      render: (r) => (
        <b style={{ color: "#c33" }}>{formatPercent(r.reject_rate)}</b>
      ),
    },
    {
      key: "total_amount",
