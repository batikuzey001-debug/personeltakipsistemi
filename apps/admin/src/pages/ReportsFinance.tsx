...
export default function ReportsFinance() {
  ...
  useEffect(() => {
    ...
    api.get<ApiListResponse<FinanceRow>>(PATH, { from, to, order, limit, offset, tz: "Europe/Istanbul" })
  ...
  const columns: Column<FinanceRow>[] = [
    ...
    { key: "total_amount", header: "Toplam Tutar", render: (r) => formatTL(r.total_amount) },
    ...
    ...(data?.rows?.some((r) => r.pending_count != null)
      ? [{ key: "pending_count", header: "Bekleyen", render: (r: FinanceRow) => (r.pending_count ?? "") } as Column<FinanceRow>]
      : []),
  ];
