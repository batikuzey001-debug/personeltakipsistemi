...
export default function AdminTaskTemplates() {
  ...
  async function load() {
    ...
    } catch (e: any) { setErr(e?.message || "Şablonlar alınamadı"); setRows([]); }
    finally { setLoading(false); }
  }
  useEffect(()=>{ load(); }, []);

  const filtered = useMemo(() => rows, [rows]); // server-side filtre kullanıyoruz
  ...
  async function bulkAdd() {
    ...
    try {
      ...
      setBulkText(""); setMsg("Şablonlar eklendi (bugün için görevler üretildi)."); setTimeout(()=>setMsg(""),1200);
      ...
      <div style={card}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"minmax(260px,1fr) 140px 160px 1fr", gap:10, alignItems:"end" }}>
          ...
          <div><div style={label}>Vardiya</div>
            <select value={bulkShift} onChange={(e)=>setBulkShift(e.target.value)}><option value="">—</option>{SHIFTS.map(s=><option key={s}>{s}</option>)}</select>
          </div>
          ...
          <div style={{ textAlign:"right" }}><button style={btnPrimary} onClick={bulkAdd} disabled={savingBulk || !bulkText.trim()}>{savingBulk?"Ekleniyor…":"Ekle (bugüne üret)"}</button></div>
        </div>
      </div>
          ...
          <div key={t.id} style={{ border:"1px solid #eef1f5", borderRadius:10, padding:10, marginBottom:8 }}>
            {editId === t.id ? (
              <div style={rowGrid}>
                ...
                <div><div style={label}>Departman</div>
                  <select value={eDept} onChange={(e)=>setEDept(e.target.value)}><option value="">—</option><option>Admin</option><option>Finans</option><option>Bonus</option><option>LC</option></select>
                </div>
              ...
              <div style={rowGrid}>
                ...
                <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
                  <span style={{ padding:"2px 8px", borderRadius:999, border:`1px solid ${t.is_active?"#bfe8d1":"#e5e7eb"}`, background: t.is_active?"#e7f7ee":"#fff", color: t.is_active?"#166534":"#374151", fontSize:12, fontWeight:700 }}>
