...
type DailyRow = {
  ...
  dept?: string;            // "bonus" | "finance" | "livechat" ...
  employee_code?: string;   // "RD-001"
  employee_name?: string;   // "İlker"
  total?: number;           // işlem adedi
  avg_first_sec?: number;   // ort. ilk yanıt (sn)
  avg_close_sec?: number;   // ort. sonuçlandırma (sn)
  trend?: number;           // yüzde veya delta (opsiyon)
  [k: string]: any;         // backend'in ek alanları
};

const PATH = "/reports/daily"; // Sende farklıysa burayı değiştir

function useQueryDefaults() {
  const [params, setParams] = useSearchParams();

  // URL’den oku, yoksa varsayılan at
...
export default function ReportsDaily() {
  ...
  useEffect(() => {
    ...
    setErr(null);
    api
      .get<ApiListResponse<DailyRow>>(PATH, {
        from,
        to,
        order,         // ör: "-total" ya da "date"
        limit,
        offset,
        tz: "Europe/Istanbul",
      })
