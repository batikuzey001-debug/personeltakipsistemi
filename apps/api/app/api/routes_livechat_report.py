...
def _date(d: str) -> str: return d[:10]
def _iso_bounds(day: str) -> tuple[str, str]:
    d = _date(day); return f"{d}T00:00:00Z", f"{d}T23:59:59Z"
...
async def _agents(c: httpx.AsyncClient):
    ...
    j = r.json(); return (j.get("items") or j.get("agents") or j) if isinstance(j, dict) else j
...
async def _list_chats_all(c: httpx.AsyncClient, f0: str, t1: str, page_limit: int = 5, page_size: int = 100):
    ...
    for _ in range(page_limit):
        ...
        j = r.json()
        items = j.get("chats") or j.get("items") or j.get("chats_summary") or []  # ✅ chats_summary eklendi
...
def _assign_agent_email(chat: dict) -> str | None:
    ...
    # ✅ tüm event tiplerinde author_id e-posta yakala
...
async def agents_summary(date_from: str = Query(...), date_to: str = Query(...)):
    ...
    async with httpx.AsyncClient() as c:
        ...
        rows = [{
            "agent_id": aid, "name": meta.get(aid, {}).get("name"),
            "role": meta.get(aid, {}).get("role"),
